...
extern "C" {
    ...
    // Automatically detect platform if not explicitly defined
#if !defined(TRAY_APPINDICATOR) && !defined(TRAY_APPKIT) && !defined(TRAY_WINAPI)
...
#define TRAY_APPINDICATOR_ID "tray-id"
    ...
    static void _tray_menu_cb(GtkMenuItem* item, gpointer data) {
        (void)item;
        struct tray_menu* m = static_cast<struct tray_menu*>(data);
        m->cb(m);
    }
...
#include <limits.h>
    ...
    static id _tray_menu(struct tray_menu* m) {
        id menu = objc_msgSend(reinterpret_cast<id>(objc_getClass("NSMenu")), sel_registerName("new"));
        ...
        for (; m != NULL && m->text != NULL; m++) {
            if (strcmp(m->text, "-") == 0) {
                objc_msgSend(menu, sel_registerName("addItem:"),
                    objc_msgSend(reinterpret_cast<id>(objc_getClass("NSMenuItem")), sel_registerName("separatorItem")));
            }
            else {
                id menuItem = objc_msgSend(reinterpret_cast<id>(objc_getClass("NSMenuItem")), sel_registerName("alloc"));
                ...
                objc_msgSend(menuItem, sel_registerName("initWithTitle:action:keyEquivalent:"),
                    objc_msgSend(reinterpret_cast<id>(objc_getClass("NSString")), sel_registerName("stringWithUTF8String:"), m->text),
                    sel_registerName("menuCallback:"),
                    objc_msgSend(reinterpret_cast<id>(objc_getClass("NSString")), sel_registerName("stringWithUTF8String:"), ""));
                ...
                objc_msgSend(menuItem, sel_registerName("setRepresentedObject:"),
                    objc_msgSend(reinterpret_cast<id>(objc_getClass("NSValue")), sel_registerName("valueWithPointer:"), m));
    ...
    static void menu_callback(id self, SEL cmd, id sender) {
        struct tray_menu* m =
            static_cast<struct tray_menu*>(objc_msgSend(objc_msgSend(sender, sel_registerName("representedObject")),
    ...
    static int tray_init(struct tray* tray) {
        pool = objc_msgSend(reinterpret_cast<id>(objc_getClass("NSAutoreleasePool")),
            sel_registerName("new"));

        objc_msgSend(reinterpret_cast<id>(objc_getClass("NSApplication")),
        ...
        class_addMethod(trayDelegateClass, sel_registerName("menuCallback:"), reinterpret_cast<IMP>(menu_callback), "v@:@");
        objc_registerClassPair(trayDelegateClass);

        id trayDelegate = objc_msgSend(reinterpret_cast<id>(trayDelegateClass),
            sel_registerName("new"));
        ...
        statusBar = objc_msgSend(reinterpret_cast<id>(objc_getClass("NSStatusBar")),
    ...
    static int tray_loop(int blocking) {
        id until = (blocking ?
            objc_msgSend(reinterpret_cast<id>(objc_getClass("NSDate")), sel_registerName("distantFuture")) :
            objc_msgSend(reinterpret_cast<id>(objc_getClass("NSDate")), sel_registerName("distantPast")));

        id event = objc_msgSend(app, sel_registerName("nextEventMatchingMask:untilDate:inMode:dequeue:"),
            ULONG_MAX,
            until,
            objc_msgSend(reinterpret_cast<id>(objc_getClass("NSString")),
    ...
    static void tray_update(struct tray* tray) {
        objc_msgSend(statusBarButton, sel_registerName("setImage:"),
            objc_msgSend(reinterpret_cast<id>(objc_getClass("NSImage")), sel_registerName("imageNamed:"),
                objc_msgSend(reinterpret_cast<id>(objc_getClass("NSString")), sel_registerName("stringWithUTF8String:"), tray->icon)));
...
#define ID_TRAY_FIRST 1000
    ...
    static LRESULT CALLBACK _tray_wnd_proc(HWND hwnd, UINT msg, WPARAM wparam,
        ...
        case WM_COMMAND:
            if (wparam >= ID_TRAY_FIRST) {
                ...
                if (GetMenuItemInfoA(hmenu, static_cast<UINT>(wparam), FALSE, &item)) {
                    struct tray_menu* menu = reinterpret_cast<struct tray_menu*>(item.dwItemData);
    ...
    static HMENU _tray_menu(struct tray_menu* m, UINT* id) {
        ...
        for (; m != NULL && m->text != NULL; m++, (*id)++) {
            if (strcmp(m->text, "-") == 0) {
                InsertMenuA(hmenu, *id, MF_SEPARATOR, TRUE, "");
            }
            else {
                MENUITEMINFOA item;
                ...
                item.wID = *id;
                item.dwTypeData = const_cast<LPSTR>(m->text);
                item.dwItemData = reinterpret_cast<ULONG_PTR>(m);
    ...
    static void tray_update(struct tray* tray) {
        ...
        SendMessage(hwnd, WM_INITMENUPOPUP, reinterpret_cast<WPARAM>(hmenu), 0);
        HICON icon;
